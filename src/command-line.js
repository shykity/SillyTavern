...
export class CommandLineParser {
    ...
    parse(args) {
        ...
        // Cloud deployment detection: PORT env var is set by cloud platforms like Scalingo, Heroku, etc.
        const envPort = process.env.PORT ? parseInt(process.env.PORT, 10) : null;
        const isValidPort = envPort !== null && !isNaN(envPort) && envPort >= 1 && envPort <= 65535;
        const isCloudDeployment = isValidPort;

        /** @type {CommandLineArguments} */
        const result = {
            configPath: configPath,
            dataRoot: dataRoot,
            port: cliArguments.port ?? (isCloudDeployment ? envPort : null) ?? getConfigValue('port', defaultConfig.port, 'number'),
            listen: cliArguments.listen ?? (isCloudDeployment ? true : null) ?? getConfigValue('listen', defaultConfig.listen, 'boolean'),
